...
```powershell
pnpm.cmd --filter @todex/desktop test --run workbench.spec.tsx ipc.test.ts desktop-run-service.test.ts node-workspace-fs.test.ts node-command-runner.test.ts openai-compatible-client.test.ts
...
- Focused tests cover `.env`, `../outside.txt`, symlink escape, shell-disabled spawn, timeout, HTTP non-2xx, oversized HTTP response, protocol repair, second-invalid stop, and a live workbench preload surface without Demo fallback.
...
No actual external model request, real API key, user repository mutation, real command, installed Windows Electron interaction, Mock HTTP end-to-end fixture, PR, CI, or release was claimed by this record. A full-suite attempt passed all non-native suites and failed 17 existing SQLite/WorkspaceHost tests because the local Node 24 runtime could not load the missing `better-sqlite3` ABI 137 binding.
