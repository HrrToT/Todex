...
本文依据仓库中的 SPEC、PLAN、AGENT_LOG、任务卡、测试输出、PR/CI 记录和发布验证记录整理。Codex 参与了规约分析、任务拆分、代码实现协作、测试审查、文档整理和本文初稿生成；项目负责人负责产品边界选择、确认关键决策、审查结果判断、PR/合并授权以及最终事实核对。本文没有写入 API Key、个人绝对路径、真实用户仓库内容或其他敏感信息。最终提交前，我会逐段核对第一人称叙述与自己的真实经历，并对全文负责。

## 提交前自检

- [x] 八个问题均已回答，并引用了多个可核查仓库证据入口。
- [x] 已明确 AI 实际参与范围。
- [x] 未写入 API Key、个人绝对路径或真实用户仓库内容。
- [x] 未把 CI、打包、Mock Demo 或静态检查写成真实模型人工验收。
- [ ] 项目负责人完成最终事实核对，并对本文结论承担最终责任。
