...
- `CredentialStore` is now stateless with respect to configuration identity. `WorkspaceHost` obtains a model config by `configId`, saves an opaque UUID reference to Keytar before persisting it in `model_configs.credential_ref`, and clears SQLite only after Keytar deletion succeeds. Reopening the host resolves status from that persisted reference. Adapter failures are exposed only as `credential_unavailable`, preserve the prior reference, and never create a plaintext fallback.
