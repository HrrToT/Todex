...
**状态：** 历史发布范围已完成，Live Agent 的人工验收由 T-014 单独收口。`v0.1.0`
（`edd996b78520b06ca6f6c9ee7f03d828efacaa08`）及其 Windows workflow
`31562649008` 已形成最初的 NSIS 安装器和 `latest.yml`；其 Renderer 白屏问题由
`v0.1.1` 修复，CI 为 `31570728577`。T-013 的真实桌面 Agent 经 PR #19 合入
`f9dcd3a8368b32fb14418bb1e05dcdc1e20ada61`（候选 head
`bc405e38b5220640d5fa92948ad0e90c527699f9`，CI `31708987042`），随后 PR #20
合入 `b42c1054b9b7906bc6ba90fcec8348cedb62b26c`，PR #21 合入
`e10e597e41a7dd489895bcc374a863db80cef7e8`，CI `31769832842` 并发布 `v0.1.2`。
PR #22 以 `dfcaf5dca5a754b30a1a3791a307d0ac3000401a` 合入最终准备文档，CI
`31785166883` 通过。Render Mock Demo 保持公开固定场景边界。
...
时只显示固定诊断页，sandbox preload 改为 CommonJS，安装包校验器验证主进程、
preload、live Renderer 和 run IPC bridge，release 校验器验证版本、安装器名和 SHA-512。

已在本地复验 `typecheck`、`lint`、`build`、`demo:mechanisms`、`smoke:preload` 和
...
保留既有 `Projreq/通用.md` 作为此前任务的历史课程依据，并纳入
`Projreq/通用要求新版.md` 作为本次下发的补充要求。两版差异仅为：新版增加“8.4 下发”
说明，并删除“反思报告必须由学生本人撰写，禁止使用 AI 代写”的一句。为不降低项目的
证据标准，`REFLECTION.md` 继续要求学生本人核对事实、承担结论并披露 AI 协助范围；本次
没有凭新版文字将人工验收、CI 或发布证据放宽为可选。
